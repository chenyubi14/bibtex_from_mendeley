...
import re
import sys
journal_txt = 'journal_list.txt'
try:
    # input the whole content of a bib file as a very long string
    journal_txt = sys.argv[1]
    bibtexdb = ''.join(open(sys.argv[2], 'r', errors='ignore').readlines())
#    print (bibtexdb)
except IndexError:
...
fr = open(journal_txt, 'r', errors='ignore')
line = fr.readline()
...
newfilename = sys.argv[2] # 'library_abbreviated.bib'
with open(newfilename, 'w') as outfile:
    outfile.write(bibtexdb)
    print("New bib file saved as %s" % newfilename )
